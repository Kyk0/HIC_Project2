...
function Kitchen() {
  ...
  async function handleAdd() {
    ...
    try {
      const newItem = await addItem({ ingredient_id: selectedIng.id, status: activeTab, quantity_text: quantity || null });
  ...
  async function handleMoveChecked() {
    try {
      ...
      setItems(prev => prev.map(i =>
        i.status === "shopping" && i.is_checked ? { ...i, status: "have", is_checked: false } : i
      ));
    } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-stone-50">
          ...
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="flex bg-white border border-stone-200 rounded-xl p-1">
              <button
                ...
                className={"px-5 py-2 rounded-lg text-sm " + (activeTab === "have" ? "bg-orange-600 text-white" : "text-stone-500 hover:text-stone-800")}
              >
                Virtual Fridge
              </button>
              <button
                onClick={() => setActiveTab("shopping")}
                className={"px-5 py-2 rounded-lg text-sm " + (activeTab === "shopping" ? "bg-orange-600 text-white" : "text-stone-500 hover:text-stone-800")}
              >
                Shopping List
              </button>
            </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        {err && <div className="bg-red-50 text-red-600 p-4 rounded-xl text-sm border border-red-200 mb-6">{err}</div>}
          ...
          <div className="flex gap-3" ref={dropdownRef}>
            <div className="flex-1 relative">
              <input
                ...
                value={inputValue}
                onChange={e => { setInputValue(e.target.value); setSelectedIng(null); setShowDropdown(true); }}
              ...
              {showDropdown && inputValue && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-stone-200 rounded-lg shadow-lg z-10 max-h-60 overflow-y-auto">
                  {filteredIngredients.length > 0 ? (
                    ...
                    <div className="px-4 py-3 text-sm text-stone-400 text-center">No matching ingredients</div>
                  )}
                </div>
              )}
            </div>
          ...
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex gap-2">
              {activeTab === "shopping" && currentItems.some(i => i.is_checked) && (
                <button onClick={handleMoveChecked} className="px-3 py-1.5 bg-orange-100 text-orange-700 rounded-lg text-xs hover:bg-orange-200">
                  Move checked to Fridge
                </button>
              )}
              {currentItems.length > 0 && (
                <button onClick={() => handleClear(false)} className="px-3 py-1.5 bg-stone-100 text-stone-600 rounded-lg text-xs hover:bg-stone-200">
                  Clear all
                </button>
              )}
            </div>
          ...
          {currentItems.length === 0 ? (
            <div className="text-center py-12">
              <p className="font-serif text-stone-600 text-lg mb-1">
                {activeTab === "have" ? "Your fridge is empty" : "Shopping list is empty"}
              </p>

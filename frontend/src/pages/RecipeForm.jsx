...
function RecipeForm() {
            ...
            .then(data => {
                ...
                setIngredients(
                    data.ingredients?.length
                        ? data.ingredients.map(i => (typeof i === "object" ? i.original_text || "" : i))
                        : [""]
                );
    ...
    function handleIngredientChange(i, v) {
        const arr = [...ingredients];
        arr[i] = v;
        setIngredients(arr);
    }

    function handleInstructionChange(i, v) {
    ...
    async function handleSubmit(e) {
        ...
        const payload = {
            ...
            meal_types: form.meal_types ? form.meal_types.split(",").map(t => t.trim()).filter(Boolean) : [],
        };
        ...
        try {
            if (isEdit) {
                ...
                if (updated.id) nav("/recipe/" + updated.id);
                else setErr(updated.detail || "Failed to update recipe.");
            } else {
                const created = await createRecipe(payload);
                if (created.id) nav("/recipe/" + created.id);
                        ...
                        <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-stone-200 rounded-xl bg-stone-50 hover:bg-stone-100">
                            ...
                            {form.image_url ? (
                                ...
                                <div className="relative w-full max-w-sm rounded-lg overflow-hidden border border-stone-200">
                                    <img src={form.image_url} alt="Preview" className="w-full h-auto object-cover" />
                                    <button
                                        type="button"
                        ...
                        <div className="flex flex-col gap-5">
                            ...
                            <div className="grid grid-cols-4 gap-5">
                                <div>
                                    ...
                                    <input type="number" name="prep_time_minutes" value={form.prep_time_minutes} onChange={handleChange} placeholder="15" className="w-full px-4 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-orange-300" />
                                </div>
                                <div>
                                    <label className="block text-sm text-stone-600 mb-1">Cook (mins)</label>
                                    <input type="number" name="cook_time_minutes" value={form.cook_time_minutes} onChange={handleChange} placeholder="45" className="w-full px-4 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-orange-300" />
                                </div>
                                <div>
                                    <label className="block text-sm text-stone-600 mb-1">Servings</label>
                                    <input type="number" name="servings" value={form.servings} onChange={handleChange} placeholder="4" className="w-full px-4 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-orange-300" />
                                </div>
                                <div>
                                    ...
                                    <input type="number" name="calories_per_serving" value={form.calories_per_serving} onChange={handleChange} placeholder="350" className="w-full px-4 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-orange-300" />
                                </div>
                            </div>

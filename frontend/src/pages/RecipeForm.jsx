...
function RecipeForm() {
  ...
  function handleImageUpload(e) {
    ...
    reader.onloadend = () => { setFormData(prev => ({ ...prev, image_url: reader.result })); setErr(null); };
  ...
  if (loading) return <div className="p-8 text-stone-500 text-center mt-20">Loading recipe...</div>;
    ...
    <div className="min-h-screen bg-stone-50">
      <div className="bg-orange-50 px-6 pt-28 pb-8 border-b border-stone-200">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          ...
          <button type="button" onClick={() => navigate(-1)} className="text-stone-500 hover:text-stone-800 text-sm">
            ← Back
          </button>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-8">
            ...
            <div className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-stone-200 rounded-xl bg-stone-50 hover:bg-stone-100">
              {formData.image_url ? (
                <div className="relative w-full max-w-sm rounded-lg overflow-hidden border border-stone-200">
                  ...
                  <button type="button" onClick={() => setFormData(f => ({ ...f, image_url: "" }))} className="absolute top-2 right-2 w-8 h-8 bg-white text-stone-700 rounded-full flex items-center justify-center shadow font-bold text-xs">
                    X
                  </button>
                </div>
              ) : (
                <div className="text-center">
                  ...
                  <p className="text-stone-500 mb-4 text-sm">Add a photo of your finished dish</p>
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-white border border-stone-200 rounded-lg text-sm text-stone-700 hover:bg-stone-50">
                    Select an Image
                  </button>
            ...
            <div className="flex flex-col gap-5">
              ...
              <div className="grid grid-cols-2 gap-5">
                <div>
                  ...
                  <input name="cuisine" value={formData.cuisine} onChange={handleChange} placeholder="Italian" className="w-full px-4 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-orange-300" />
                </div>
                <div>
                  <label className="block text-sm text-stone-600 mb-1">Difficulty</label>
                  <select name="difficulty" value={formData.difficulty} onChange={handleChange} className="w-full px-4 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-orange-300 bg-white">
              ...
              <div className="grid grid-cols-4 gap-5">
                <div>
                  ...
                  <input type="number" name="prep_time_minutes" value={formData.prep_time_minutes} onChange={handleChange} placeholder="15" className="w-full px-4 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-orange-300" />
                </div>
                <div>
                  <label className="block text-sm text-stone-600 mb-1">Cook (mins)</label>
                  <input type="number" name="cook_time_minutes" value={formData.cook_time_minutes} onChange={handleChange} placeholder="45" className="w-full px-4 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-orange-300" />
                </div>
                <div>
                  <label className="block text-sm text-stone-600 mb-1">Servings</label>
                  <input type="number" name="servings" value={formData.servings} onChange={handleChange} placeholder="4" className="w-full px-4 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-orange-300" />
                </div>
                <div>
                  <label className="block text-sm text-stone-600 mb-1">Calories</label>
                  <input type="number" name="calories_per_serving" value={formData.calories_per_serving} onChange={handleChange} placeholder="350" className="w-full px-4 py-2 border border-stone-200 rounded-lg text-sm focus:outline-none focus:border-orange-300" />
                </div>
              </div>
            </div>
          </div>
              ...
              <div className="flex-1 flex flex-col gap-3">
                {ingredients.map((ing, i) => (
                  <div key={i} className="flex gap-2 items-center group">
                    ...
                    />
                    <button type="button" onClick={() => setIngredients(ingredients.filter((_, idx) => idx !== i))} className="p-1.5 text-stone-300 hover:text-red-400 opacity-0 group-hover:opacity-100">✕</button>
                  </div>
                ))}
              </div>
              <button type="button" onClick={() => setIngredients([...ingredients, ""])} className="mt-4 self-start px-4 py-2 bg-stone-50 border border-stone-200 text-stone-600 rounded-lg text-sm hover:bg-stone-100">
                + Add Ingredient
              </button>
            </div>
              ...
              <div className="flex-1 flex flex-col gap-3">
                {instructions.map((inst, i) => (
                  <div key={i} className="flex gap-2 items-start group">
                    ...
                    />
                    <button type="button" onClick={() => setInstructions(instructions.filter((_, idx) => idx !== i))} className="p-1.5 text-stone-300 hover:text-red-400 opacity-0 group-hover:opacity-100 mt-1">✕</button>
                  </div>
                ))}
              </div>
              <button type="button" onClick={() => setInstructions([...instructions, ""])} className="mt-4 self-start px-4 py-2 bg-stone-50 border border-stone-200 text-stone-600 rounded-lg text-sm hover:bg-stone-100">
                + Add Step
              </button>
            </div>
          </div>
          ...
          <div className="flex justify-end gap-4">
            <button type="button" onClick={() => navigate(-1)} className="px-6 py-3 bg-white text-stone-600 rounded-lg hover:bg-stone-50 border border-stone-200">
              Cancel
            </button>
            <button type="submit" disabled={saving} className="px-8 py-3 bg-orange-600 text-white rounded-lg hover:bg-orange-700 disabled:opacity-50">
